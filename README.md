...
The crop is saved as a reusable profile. Movement in presenter video, chat, or other parts of the recording stays outside slide detection and outside the final screenshots.

## Quick start

Install the application, then open a terminal in the folder containing `lecture.mp4` and `lecture.vtt`.

For the first recording layout, select the slide area and create the document:
...
Drag around the slides during selection, then press Enter or Space. For every later lecture with the same layout, only two commands are needed:
...
Review the four-column contact sheet before building to spot false or duplicate transitions:

![A contact sheet generated by slides-docx showing four detected presentation slides](docs/assets/contact-sheet.jpg)

`lecture.slide-times.txt` remains a simple, editable list of slide boundaries. The final DOCX alternates between a landscape slide page and a portrait page of normal editable transcript text.
